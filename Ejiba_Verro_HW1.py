...
def is_palindrome(word):
    ''' A function to open and read the file to be checked for palindrome
    
        Parameter:
        word - string in the sentence to check if it's a palindrome 
    '''
...
def speak_ICAO(text, letterpause = .25, wordpause = .5):
    ''' Your task in this exercise is to write a procedure speak_ICAO()able to 
        ...
        Parameters:
        text - string
        letterpause - pause after each letter
        wordpause - pause after a word
    '''
...
def hapax(filename):
    ''' Solution to number 5: Print all hapaxes in a given file name text
        A function that find all the hapaxes given the following text check for 
        the word that appears once 
...
def anagramgenerator(choice):
    ...
    word = color[num] #hides the word that it at the position num -- from random selection
...
def anagram_sets(filename): #parameter --  filename, a file with the list of words
    ...
    """
        ...
        Parameter:
        filename - a file with the list words
    """
...
def alternade(filename):
    ''' An alternade is a word in which its letters, taken alternatively in a strict 
        ...
        Parameter:
        filename - a file from directory
    '''
